...
filtered_data = dataset[['Age(years)', 'Data_Value']].dropna()
average_age_data = filtered_data.groupby('Age(years)')['Data_Value'].mean().reset_index()

# Plotting
plt.figure(figsize=(12, 5))
bar_plot = plt.bar(average_age_data['Age(years)'], average_age_data['Data_Value'], color='orange')

# Axes labels and title
plt.xlabel('Age Groups')
plt.ylabel('Average Obesity Percentage')
plt.title('Average Obesity Percentage by Age Group')
